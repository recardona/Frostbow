...
namespace BoltFreezer.PlanSpace
{
    public class PlanSpacePlanner : IPlanner
    {
        ...
        public void WriteToFile(long elapsedMs, Plan plan)
        {
            ...
            var namedData = new List<Utilities.Tuple<string, string>>
                        {
                            new Utilities.Tuple<string, string>("problem", problemNumber.ToString()),
                            new Utilities.Tuple<string, string>("selection", selection.ToString()),
                            new Utilities.Tuple<string, string>("search", search.ToString()),
                            new Utilities.Tuple<string,string>("runtime", elapsedMs.ToString()),
                            new Utilities.Tuple<string, string>("opened", opened.ToString()),
                            new Utilities.Tuple<string, string>("expanded", expanded.ToString()),
                            new Utilities.Tuple<string, string>("primitives", primitives.ToString() ),
                        };

            var file = directory + problemNumber.ToString() + "-" + search.ToString() + "-" + selection.ToString() + ".txt";
            using (StreamWriter writer = new StreamWriter(file, false))
            {
                foreach (Utilities.Tuple<string, string> dataItem in namedData)
                {
